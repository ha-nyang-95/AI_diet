...
Story 1.6 — `onboarded_at` 멱등 set + `/v1/users/me` 노출 회귀 4 케이스 추가.
...
# --- Story 1.6 — `onboarded_at` 멱등 set + GET /me 노출 (AC #5, #6, #9) ---


async def test_profile_post_sets_onboarded_at_first_time(
    client: AsyncClient,
    user_factory: UserFactory,
    consent_factory: ConsentFactory,
) -> None:
    """신규 사용자 첫 POST → DB ``onboarded_at`` SET + GET /me 응답에 ISO datetime."""
    user = await user_factory()
    await consent_factory(user)
    ...
    me = await client.get("/v1/users/me", headers=auth_headers(user))
    assert me.status_code == 200
    body = me.json()
    assert body["onboarded_at"] is not None
    # ISO 8601 datetime parse 가능 — Pydantic이 직렬화한 형식 그대로.
    datetime.fromisoformat(body["onboarded_at"].replace("Z", "+00:00"))


async def test_profile_post_preserves_onboarded_at_on_re_post(
    client: AsyncClient,
    user_factory: UserFactory,
    consent_factory: ConsentFactory,
) -> None:
    """첫 POST 후 ``onboarded_at`` 캡처 → 재 POST → 값 동일 + ``profile_completed_at`` 갱신."""
    user = await user_factory()
    await consent_factory(user)
    ...
    first_profile_completed_at = first.json()["profile_completed_at"]

    me_first = await client.get("/v1/users/me", headers=auth_headers(user))
    onboarded_at_first = me_first.json()["onboarded_at"]
    assert onboarded_at_first is not None

    # DB ``func.now()`` 시계 차이 보장 (Postgres NOW()는 statement timestamp).
    ...
    second_profile_completed_at = second.json()["profile_completed_at"]

    me_second = await client.get("/v1/users/me", headers=auth_headers(user))
    onboarded_at_second = me_second.json()["onboarded_at"]

    # ``onboarded_at``: 첫 POST 시점 그대로 (멱등).
    assert onboarded_at_second == onboarded_at_first
    # ``profile_completed_at``: 매 POST 갱신.
    assert second_profile_completed_at != first_profile_completed_at


async def test_profile_post_onboarded_at_idempotent_with_pre_set(
    client: AsyncClient,
    user_factory: UserFactory,
    consent_factory: ConsentFactory,
) -> None:
    """수동 SQL UPDATE로 ``onboarded_at`` 미리 set한 사용자 → POST → 값 변경 X."""
    user = await user_factory()
    await consent_factory(user)

    # user_factory는 ``onboarded_at`` 인자 미지원 — 수동 UPDATE로 미리 set.
    pre_set = datetime(2025, 1, 1, 0, 0, 0, tzinfo=UTC)
    session_maker: async_sessionmaker[AsyncSession] = app.state.session_maker
    async with session_maker() as session:
        await session.execute(update(User).where(User.id == user.id).values(onboarded_at=pre_set))
    ...
    me = await client.get("/v1/users/me", headers=auth_headers(user))
    onboarded_at = me.json()["onboarded_at"]
    parsed = datetime.fromisoformat(onboarded_at.replace("Z", "+00:00"))
    # ``COALESCE``는 기존 값 보존 — pre_set 시점 그대로.
    assert parsed == pre_set


async def test_users_me_exposes_onboarded_at_after_profile_post(
    client: AsyncClient,
    user_factory: UserFactory,
    consent_factory: ConsentFactory,
) -> None:
    """POST 후 GET /me 응답에 ``onboarded_at`` 키 노출 (UserMeResponse 회귀 차단)."""
    user = await user_factory()
    await consent_factory(user)

    me_pre = await client.get("/v1/users/me", headers=auth_headers(user))
    assert me_pre.status_code == 200
    # POST 전: onboarded_at은 null (Story 1.2 baseline 컬럼).
    assert me_pre.json()["onboarded_at"] is None
    ...
    me_post = await client.get("/v1/users/me", headers=auth_headers(user))
