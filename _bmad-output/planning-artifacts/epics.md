...
**And** **모바일 jest + React Native Testing Library 인프라 도입** (Story 4.1 CR W1 amendment, 2026-05-06) — `mobile/jest.config.ts` + `jest-expo` preset + `@testing-library/react-native` + `@testing-library/jest-native` + `jest-environment-jsdom` 추가 + `mobile/package.json` `test` 스크립트 + CI workflow 게이트 추가. **흡수할 deferred 테스트 (Story 4.1 W1 — 12건)**: `mobile/lib/__tests__/permissions.test.ts` (`useNotificationPermission` mocked status granted/denied/undetermined/provisional 4건) + `mobile/lib/__tests__/push.test.ts` (`registerForPushNotificationsAsync` simulator/granted/denied/projectId 누락 4건) + `mobile/features/notifications/__tests__/PermissionDeniedView.test.tsx` (canAskAgain true/false 렌더링 2건) + `mobile/features/notifications/__tests__/api.test.ts` (Zod schema 검증 2건). **추가 hygiene scope** (CR Edge Hunter 신호 대응): `useCameraPermission`/`useMediaLibraryPermission` (Story 2.2) + 핵심 hook + UI smoke 일괄 5건 — 회귀 가드 baseline 확립.
