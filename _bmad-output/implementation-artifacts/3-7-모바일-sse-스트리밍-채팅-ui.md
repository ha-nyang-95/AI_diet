...
- [x] **Task 13 — `[meal_id].tsx` ChatStreaming 통합 (AC14)**
  ...
  - [x] 13.5 `summary !== null` 분기는 기존 그대로 유지 + ChatStreaming 미마운트(이미 분석 완료된 meal은 summary 카드 + DisclaimerFooter 노출). 단, 본 스토리는 *meal_analyses 영속화 OUT*이므로 GET /v1/meals 응답 `analysis_summary` 항상 null — 결국 본 스토리에서는 *최초 분석* 화면만 노출되고 새로고침 시 다시 *"AI 분석 시작"* 진입(허용 — Story 8.4 polish가 영속화 + 캐시 hit으로 한 번만 분석).
