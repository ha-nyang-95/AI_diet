...
| 2026-05-01 | Amelia (claude-opus-4-7[1m]) | Story 3.1 CR 완료 → done. 3-layer 병렬 리뷰(Blind Hunter + Edge Case Hunter + Acceptance Auditor)에서 12 patch / 9 defer / 16 dismiss 분류, 12 patch 모두 적용: (1) **D6/AC4** `run_food_seed` per-batch `session.commit()` — 부분 실패 시 마지막 commit 이전 chunk 보존, (2) `_format_vector_literal` NaN/Inf isfinite 가드 + `_insert_batch` `json.dumps(allow_nan=False)` try/except — 1행 NaN으로 batch 전체 abort 차단, (3) `_read_zip_fallback` `encoding="utf-8-sig"` — Excel-export BOM silent skip 차단, (4) source_id 빈 행 → `MFDS-ZIP-{sha1[:12]}` 안정 해시 — CSV 행 재정렬 시 ON CONFLICT 데이터 손상 차단, (5) `_normalize_category` NFC + middle-dot(U+30FB/U+318D → U+00B7) + 공백 strip + 5페이지 후 zero-match 시 `FoodSeedAdapterError` — 카테고리 silent zero-rows 가드, (6) `_is_strict_mode`/`_is_graceful_environment` 대소문자 무시 + `1`/`true`/`yes`/`on` — 12-factor 정합, (7) `seed_food_db.main` graceful WARN을 stdout으로 — AC11 시나리오 2 정합, (8) `aliases_seed` 모듈 import 시 `len(FOOD_ALIASES) >= ALIASES_MIN_COUNT` assert + `processed == len(FOOD_ALIASES)` 검증 — pre-existing 행으로 통과되는 silent regression 차단, (9) `fetch_food_items` 마지막 부분 페이지 break — max_pages 상한 무의미 fetch 차단, (10) `_extract_items` single-dict envelope 처리 + 비-dict skip 가시화 로그, (11) `_embed_batch` 길이 mismatch 시 `[None]*len(items)` graceful — OpenAI 부분 응답 batch abort 차단, (12) `test_bootstrap_seed_prod_fail_fast` + `test_bootstrap_seed_dev_strict_opt_in` 통합 테스트 2건 신규 — AC6 5케이스 완전 커버. defer 9건은 `deferred-work.md` DF75-DF83에 적재. Gate: 282 passed / 1 skipped / coverage 84.21% / ruff·format clean / mypy 0 신규. Status: review → done. |
...
CR 실행 — 2026-05-01 (Amelia / 3-layer parallel: Blind Hunter + Edge Case Hunter + Acceptance Auditor). Triage: 12 patch / 9 defer / 16 dismiss.

#### Patch (must-fix)

- [x] [Review][Patch] **D6/AC4 violation — per-batch commit 누락** [api/app/rag/food/seed.py:run_food_seed L320-325] — batch 루프가 commit 없이 5 chunk를 돌고 caller(`scripts/seed_food_db.py:_run_seeds`)가 한 번만 commit. spec D6: *"5 batch / 5 commit ... 부분 실패 시 마지막 commit 이전 batch는 보존"* 위반. Fix: `await session.commit()`을 batch 루프 안에 추가. (sources: blind+auditor)
- [x] [Review][Patch] **NaN/Inf safety — vector + nutrition jsonb** [seed.py:_format_vector_literal L187-191, _nutrition_payload L163-184, _insert_batch L208] — `format(NaN, '.7g')="nan"` → pgvector `CAST AS vector` 실패; `json.dumps` allow_nan=True 기본 → `NaN` jsonb invalid. Fix: `_format_vector_literal`에 `math.isfinite` 검사로 None coerce, `_nutrition_payload`의 `json.dumps`에 `allow_nan=False` + try/except. (sources: blind+edge)
- [x] [Review][Patch] **CSV BOM — Excel-export utf-8-sig 무시** [seed.py:_read_zip_fallback L127] — `encoding="utf-8"` → BOM이 있으면 `﻿name` 키로 row.get("name") None → 전 행 silent skip. Fix: `encoding="utf-8-sig"`. (source: edge)
- [x] [Review][Patch] **CSV source_id stable hash** [seed.py:_read_zip_fallback L133] — `f"MFDS-ZIP-{idx}"` synthetic ID는 CSV 행 재정렬 시 다른 음식의 source_id와 충돌 → ON CONFLICT UPDATE가 잘못된 행 덮어쓰기(데이터 손상). Fix: `source_id`가 비면 `f"MFDS-ZIP-{hashlib.sha1(name.encode()).hexdigest()[:12]}"` 같은 안정적 해시 사용. (source: edge)
- [x] [Review][Patch] **Korean category whitelist 정규화 + zero-match guard** [api/app/adapters/mfds_openapi.py:_raw_to_standard L996-1025, fetch_food_items L1144-1154] — `MFDS_KOREAN_FOOD_CATEGORIES` 화이트리스트가 U+00B7(`·`) 정확 매칭만 허용 → API가 U+30FB(`・`) / 공백 변종 / `None` category 반환 시 silent zero rows. Fix: `unicodedata.normalize("NFC", category).strip()` + `pages_fetched > 5 and len(collected) == 0`이면 `FoodSeedAdapterError("category whitelist matched zero rows")`. (sources: blind+edge)
- [x] [Review][Patch] **`_is_strict_mode` / `_is_graceful_environment` 대소문자/추가값** [seed.py L1789-1790, L1791-1797] — `RUN_FOOD_SEED_STRICT=TRUE`(대문자) silent miss. Fix: `.lower() in {"1","true","yes","on"}` + 환경값도 lowercase 매칭. (sources: blind+edge)
- [x] [Review][Patch] **AC11 stdout 경고** [scripts/seed_food_db.py:main L83-88] — graceful 0건 WARN을 `file=sys.stderr`로 출력. spec AC11 시나리오 2: *"stdout warning"*. Fix: `file=` 인자 제거(default stdout). (sources: auditor+edge)
- [x] [Review][Patch] **Aliases count gate via len(FOOD_ALIASES)** [api/app/rag/food/aliases_seed.py:seed_food_aliases L1684-1690] — `SELECT count(*)` gate가 pre-existing 행 포함 → FOOD_ALIASES dict이 50→40으로 줄어도 stale DB 데이터로 통과. Fix: 모듈 import 시 `assert len(FOOD_ALIASES) >= ALIASES_MIN_COUNT` + DB 확인은 inserted+updated == len(FOOD_ALIASES) 검증. (sources: blind+edge)
- [x] [Review][Patch] **Last partial page break 누락** [api/app/adapters/mfds_openapi.py:fetch_food_items L1126-1138] — 마지막 부분 페이지 감지 없이 `max_pages=100`까지 무의미 fetch. Fix: `if len(raw_items) < page_size: break`. (source: edge)
- [x] [Review][Patch] **`_extract_items` single-dict + 필터링 카운트 로그** [api/app/adapters/mfds_openapi.py:_extract_items L1042-1056] — `body["data"]`가 dict(단일 객체) 시 `[body["data"]]`로 감싸지 않고 빈 리스트 반환; non-dict 항목 silent drop. Fix: `if isinstance(body["data"], dict): return [body["data"]]` + `if len(raw) != len(filtered): logger.warning(...)`. (sources: blind+edge)
- [x] [Review][Patch] **`_embed_batch` length 검증** [seed.py:_embed_batch L224-241, run_food_seed L322-323] — `zip(items, embeddings, strict=True)`인데 OpenAI 부분 응답(undocumented but possible) 시 ValueError로 batch 전체 abort. Fix: `_embed_batch` 종료 직전 `if len(embeddings) != len(items): return [None] * len(items)` 검증 + warn 로그. (source: edge)
- [x] [Review][Patch] **AC6 통합 테스트 케이스 누락 — prod fail-fast + dev strict opt-in** [api/tests/integration/test_bootstrap_seed.py] — spec AC6은 5 케이스 명시(정상/dev graceful/**prod fail-fast**/**dev strict opt-in**/멱등). 현 3 케이스만 구현(정상/dev graceful/멱등). Fix: `test_bootstrap_seed_prod_fail_fast`(environment=prod + key 미설정 → exit 1 + Sentry tag) + `test_bootstrap_seed_dev_strict_opt_in`(environment=dev + RUN_FOOD_SEED_STRICT=1 + key 미설정 → exit 1) 2 케이스 추가. (source: auditor)

#### Defer (out-of-scope, 추적 유지)

- [x] [Review][Defer] **`updated_at` ON UPDATE DB-level 트리거** [api/alembic/versions/0010_food_nutrition_and_aliases.py L731-736, L772-777] — ORM `onupdate`만 — psql 직접 UPDATE는 자동 갱신 X — Story 8(운영 polish) 시점 trigger 또는 generated column 추가
- [x] [Review][Defer] **`_SEED_CSV_PATH = __file__.parents[3]` 경로 fragile** [seed.py L1755-1759] — Story 8 hardening — `settings.seed_data_dir` 도입 + startup validate
- [x] [Review][Defer] **`MFDS_RAW_KEY_MAP` dead const 정리** [api/app/adapters/mfds_openapi.py L863-877 vs _raw_to_standard L996-1025] — Story 8 polish — const-driven 매핑으로 SOT 일원화
- [x] [Review][Defer] **HNSW partial index `WHERE embedding IS NOT NULL`** [api/alembic/versions/0010_food_nutrition_and_aliases.py L745-752] — Story 3.3 retrieve_nutrition 검색 perf 측정 후 결정
- [x] [Review][Defer] **`_get_client` MFDS API key 회전 캐시 무효화** [api/app/adapters/mfds_openapi.py:_get_client L955-975] — Story 8 prod hardening (rolling deploy 시 key rotation 시나리오)
- [x] [Review][Defer] **Migration 부분실패 복구** [0010 upgrade/downgrade] — `op.execute("DROP INDEX IF EXISTS ...")` 패턴 — Story 8 hardening
- [x] [Review][Defer] **CSV header schema 검증** [seed.py:_read_zip_fallback] — 운영자 typo 보호 — Story 8 polish
- [x] [Review][Defer] **`bootstrap_seed.py` outer Sentry capture** [scripts/bootstrap_seed.py L32-36] — ImportError edge case — Story 8 polish
- [x] [Review][Defer] **bootstrap test `>=` vs `==` 정확 매칭** [api/tests/integration/test_bootstrap_seed.py:test_bootstrap_seed_normal] — minor; over-insertion 회귀 가드 — Story 8 polish
