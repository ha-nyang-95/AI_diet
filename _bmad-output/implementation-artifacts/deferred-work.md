...
- **DF46 — manual retry granular `attempts` reset 정책** [mobile/lib/offline-queue.ts flushQueue] — 본 스토리 baseline은 manual retry 시 attempts reset 미적용(누적 유지). *attempts cap 도달 후 manual retry granular reset*은 Story 8 polish — 사용자 manual 재시도 무한 부재 차단. **재검토 시점**: Story 8 polish.
