...
## Deferred from: code review of 6-2-구독-해지-영수증-이용내역 (2026-05-09)

- **DF135 — `get_subscription` active branch `expires_at > now()` 게이트 부재** [api/app/api/v1/payments.py:333-345] — 구현은 `or_(status=='active', (status=='cancelled') & (expires_at > now))`로 active branch에 expires_at gate 미적용. spec line 89는 active/cancelled 모두 expires_at gate 명시. webhook race window에서 active-but-expired row 노출 가능. 본 스토리 baseline은 webhook 부재 = race window 미존재. **재검토 시점**: Story 6.3 webhook으로 `status=expired` 전이가 정식 처리되는 시점.
- **DF136 — 결제 화면 401 글로벌 recovery 핸들러 부재** [mobile/app/(tabs)/settings/payment-history.tsx + web/src/features/payments/PaymentHistoryTable.tsx] — 401 발생 시 generic 에러 카드만 표시, 자동 redirect / 토큰 재발급 경로 부재. cross-cutting concern으로 화면별 패치 대신 글로벌 axios/fetch interceptor 권장. **재검토 시점**: Story 8.x 운영 polish (Sentry/SLA hardening 동반).
- **DF137 — OpenAPI 400 응답 스키마 일괄 등록 (`POST /v1/payments/cancel` 404/409 + `GET /v1/payments/history` 400)** [api/app/api/v1/payments.py + auto-generated mobile/lib/api-client.ts + web/src/lib/api-client.ts] — global `validation_exception_handler`가 422 → 400(`code=validation.error`) 변환하지만 OpenAPI 명세는 여전히 422만 선언. `cancel` 404/409 응답은 `content?: never`로 RFC 7807 본문 schema 부재. DF133 baseline 연속(Story 5.2부터 누적). 자동 생성 클라이언트 타입 부정확하나 런타임 동작 정상. **재검토 시점**: Story 8.4 polish — 글로벌 핸들러 OpenAPI extra responses 일괄 등록 시.


## Deferred from: code review of 7-2-관리자-사용자-검색-이력-수정 (2026-05-11)

- **DF138 — admin 사용자 상세 페이지 식단·피드백 UI cursor pagination** [web/src/features/admin/UserMealsTable.tsx:48-57, UserMealAnalysesTable.tsx:42-51] — backend는 `next_cursor` 응답 제공하나 UI는 `?limit=50` 하드코딩 후 truncate. 51건 이상 사용자의 데이터를 admin이 볼 수 없음. "더 보기" 버튼 또는 무한 스크롤 추가. **재검토 시점**: Story 7.4 admin polish 또는 운영 데이터 51건 초과 케이스 발생 시.
- **DF139 — admin SSR fetch의 cookie ↔ `Authorization: Bearer` dual flow audit transport 분기** [web/src/app/(admin)/admin/users/[user_id]/page.tsx:33-37, web/src/lib/admin-api.ts] — SSR은 header forward, 클라이언트는 cookie. Story 7.3 audit_admin_action dep 도입 시 두 transport를 audit log에서 어떻게 구별/통합할지 결정 필요. **재검토 시점**: Story 7.3 audit log 자동 기록 도입.
- **DF140 — admin endpoint의 cross-admin action 거버넌스 가드** [api/app/api/v1/admin.py:627-725] — admin A가 다른 admin B의 식단/프로필 변경 시 별도 정책/가드 없음. last-admin lockout 검증, admin-on-admin action audit 분리 등 정책 결정 forward. **재검토 시점**: Story 7.3 audit log 도입 또는 admin 2인 이상 운영 시점.
- **DF141 — `app/core/masking.py` Sentry beforeSend + structlog processor 자동 적용 hook** [api/app/core/masking.py docstring] — 모듈 docstring은 Sentry/structlog 통합을 SOT로 claim하나 실제 hook 미구현. NFR-S5 자동 적용 회귀 보존 부재. **재검토 시점**: Story 8.4 운영 polish (Sentry 통합).
- **DF142 — alembic env.py 비-tx 모드 + asyncpg autocommit 분기 (CONCURRENTLY 지원)** [api/alembic/env.py + 0020/0021 expression index 마이그레이션] — 0020/0021 모두 prod-scale lock 회피 위해 `CREATE INDEX CONCURRENTLY` SOP 권장하나 현재 test infra가 `disable_ddl_transaction = True` + asyncpg + `context.begin_transaction()` 조합을 honor 못해 round-trip 회귀 가드 3건(`test_alembic_001x_round_trip`)이 깨짐. env.py에 *migration-flag-aware* 비-tx 분기 추가 후 0020/0021 모두 CONCURRENTLY 변환. Gemini Code Assist CR G1(2026-05-11) 권장 사항. **재검토 시점**: Story 8.4 운영 polish (prod 트래픽 + 100만 row 이상 시점 또는 Sentry 통합과 함께).


## Deferred from: code review of 7-3-audit-log-자동-기록-연쇄 (2026-05-11)

- **DF143 — `target_user_id` FK 위반 시 audit silent drop + attempted-vs-verified target 불일치** [api/app/services/audit_service.py:_extract_path_param_uuid + audit_logs.target_user_id FK ondelete SET NULL] — `_extract_path_param_uuid`가 path param의 UUID 형식만 검증하고 users.id 존재 여부는 미검증. nonexistent user_id 접근 시 INSERT가 FK 위반 → fail-open으로 silent drop → audit-trail에 enumeration 시도가 0건 기록. 또한 PATCH/DELETE 시 핸들러 본문이 *실제* target ownership을 검증 후 404 raise 하지만 audit row의 `target_user_id`는 *시도한* 값이라 forensic 시 misleading. DS notes 970행에 design caveat 명시 + `test_patch_user_profile_with_nonexistent_user_id_fails_open`로 invariant 문서화 완료. **해결책 후보**: (a) FK 제거 + NULL fallback (target_user_id 무결성 vs 가시성 trade-off 재고) / (b) audit row에 `success`/`http_status` 컬럼 추가 (attempt-vs-success 분리) / (c) record 시점에 SELECT로 존재 검증. **재검토 시점**: Story 8.4 운영 polish (audit DB role 분리 + audit 행 보강 시).
- **DF144 — audit_logs `actor_id ON DELETE RESTRICT` cron purge 실패 운영 알림 부재** [api/app/db/models/audit_log.py:actor_id FK ondelete RESTRICT + alembic 0022:124-127] — Story 5.2 30일 grace 도달 시 `soft_delete_purge` cron이 audit row를 가진 admin user를 hard delete 시도하면 FK ForeignKeyViolation. 현재 alert/Sentry 룰/모니터링 미구성 → cron batch 일부/전체 실패 시 GDPR 30일 purge SLA가 silent drift. 트리거 invariant + actor_email_masked 영속화 설계의 trade-off는 의도(Deviation 1 spec 920행). **해결책 후보**: (a) cron worker에서 audit-blocked user를 별도 표시 + Sentry alert / (b) admin role 보유자는 hard delete 대상 제외 정책 / (c) audit DB role 분리 시 actor_id 비-FK 컬럼화. **재검토 시점**: Story 8.4/8.5 운영 polish (audit DB role rotation SOP + cron observability).
- **DF145 — alembic 0022 downgrade가 audit_logs append-only 불변식 우회** [api/alembic/versions/0022_create_audit_logs.py:downgrade] — `op.drop_table("audit_logs")`는 append-only 트리거 BEFORE UPDATE/DELETE를 우회(trigger는 row-level만 가드)하고 모든 audit row 데이터를 silent 파괴 + ENUM `DROP TYPE` non-CASCADE라 미래 다른 테이블이 enum 참조 시 다운그레이드 중간 실패. epics.md:917 *"audit log는 변경 불가 — INSERT 외 권한 없음"* 불변식이 마이그레이션 레벨에서는 보존되지 않음. 운영 downgrade rare하나 audit DB role 분리 시 DDL 권한도 분리 필요. **해결책 후보**: (a) downgrade에 row-count guard (`SELECT count(*) FROM audit_logs > 0 → RuntimeError`) / (b) audit DB role 분리 후 마이그레이션 권한 role 분리 (alembic 실행 자체를 audit row 가진 환경에서 차단) / (c) DROP TYPE에 CASCADE 또는 use-check 추가. **재검토 시점**: Story 8.4 운영 polish (audit DB role separation + alembic 권한 분리).


## Deferred from: code review of 7-4-self-audit-화면-마스킹-원문-토글 (2026-05-11)

- **DF146 — Filters dropped on cursor pagination ("더 보기") in self-audit list** [`web/src/features/admin/AuditLogTable.tsx:108-115` + `web/src/app/(admin)/admin/audit-logs/page.tsx:22-25`] — `queryFn` URL이 `actor_id` + `limit` + `cursor`만 포함, `action`/`target_user_id`/`since` filter 미forward. 현재 UI는 filter UI 미노출(`actor_id=me&limit=50` 고정)이라 latent — 페이지 2+ 클릭 시 filter 컨텍스트가 silent 손실되는 결함이 filter UI 도입 직전까지 드러나지 않음. **해결책 후보**: (a) `useInfiniteQuery` + `getNextPageParam`에서 모든 filter param 자동 forward / (b) 별도 `useSearchParams` 기반 query string 동기화 helper로 SSR/Client 양방향 정합. **재검토 시점**: Story 8.4 admin UX polish (audit-logs filter UI 도입 + `useInfiniteQuery` 리팩토링 일괄).
- **DF147 — Initial SSR audit-logs 페이지 새로고침 affordance 부재** [`web/src/features/admin/AuditLogTable.tsx:104` `enabled: activeCursor !== null`] — 첫 페이지는 SSR로 단발 fetch되고 client query는 `activeCursor !== null`일 때만 enabled → page 1 데이터는 full reload 외에 갱신 경로 0. 동시 admin 다중 운영 시 새 audit row가 화면에 반영되지 않음. **해결책 후보**: (a) "새로고침" 버튼 + 별도 page-1 query / (b) `useInfiniteQuery` 채택 시 자동 해결(refetch first page) / (c) `refetchOnWindowFocus: true`로 page-1 query 별도 enable. **재검토 시점**: Story 8.4 admin UX polish (`useInfiniteQuery` 리팩토링과 함께).
- **DF148 — soft-deleted admin (`users.deleted_at IS NOT NULL`)이 잔존 JWT(≤8h TTL)로 self-audit/`pii-reveal` 호출 가능** [`api/app/api/deps.py:current_admin` Story 1.2 SOT, 7.4 미변경] — `current_admin`이 `role='admin'`만 검사, `deleted_at IS NULL` 가드 부재. Story 5.2 soft-delete grace 도중인 admin이 잔존 cookie로 audit row 신규 INSERT 가능 + plaintext PII 노출. **pre-existing** — 7.4 도입 결함 아니나 7.4 endpoint가 표면 확장. **해결책 후보**: (a) `current_admin`에 `User.deleted_at.is_(None)` 가드 추가 / (b) `Depends(current_active_admin)` 신규 + admin endpoint 전수 교체 / (c) JWT `iat` 기반으로 soft-delete 시점 이후 토큰 자동 만료(revoke list 또는 user.tokens_invalid_after 컬럼). **재검토 시점**: Story 8.4 admin auth hardening 또는 audit DB role 분리 SOP와 함께.
