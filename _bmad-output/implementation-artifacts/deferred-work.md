...
- **DF102 — 식약처 한식 면류 카테고리 보강 — `max_pages` 확대 + 가공식품 그룹 추가 fetch** [api/app/adapters/mfds_openapi.py] — 어댑터 재정확화(PR #22) 후 시드 결과 food_nutrition 679건 수집됐으나 한식 *면류* 카테고리 0건. 원인: (a) `max_pages=100`(=10K 행) 한도로 면류 가능 row 미도달, (b) 한식 가정식 그룹(`DB_GRP_CM=D`)만 fetch — 라면 등은 가공식품 그룹(`DB_GRP_CM=P`)에 위치. 보강안: (1) `max_pages` 100→500 확대(API rate limit 분당 100회 + 일일 10K 한도 안전), (2) `_fetch_page` 호출 시 `DB_GRP_CM` 필터 풀거나 양 그룹 병렬 fetch. 예상 추가 row ~50-100건(라면/우동/냉면 등 변형). **재검토 시점**: Story 8.4 운영 polish 슬롯.
- **DF103 — 외식/배달 메뉴 핵심 50종 수동 CSV 보강 — 식약처 미지원 영역 SOT** [api/data/mfds_food_nutrition_seed.csv + api/data/README.md] — 짜장면/짬뽕/탕수육(중식) + 떡볶이/순대(분식) + 김밥 변형 다양화 등 *영업 데모 자주 등장하는 외식 메뉴*는 식약처 영양분석 대상이 아님(API 검색 totalCount=0). 현재 Self-RAG clarification 분기로 graceful 처리되지만 데모 임팩트 보강을 위해 50건 수동 CSV 입력. CSV 컬럼은 어댑터의 ZIP fallback 포맷 정합(name/category/energy_kcal/...). 데이터 출처: 농촌진흥청 식품성분 DB 또는 외식업체 영양정보 공시. **재검토 시점**: Story 8.4 또는 외주 인수 클라이언트 customization SOP — 클라이언트 도메인 메뉴(예: 병원 환자식)도 동일 경로로 보강.
