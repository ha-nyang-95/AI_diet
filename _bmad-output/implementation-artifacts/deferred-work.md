...
- **W1 — AC15 mobile ~12건 jest 단위 테스트**: spec(`push.test.ts` ~4 + `permissions.test.ts` ~4 + `PermissionDeniedView.test.tsx` ~2 + `api.ts` schema ~2)이 명시했으나 mobile baseline에 jest 인프라 부재(Story 1.x/2.x/3.x tsc + lint 게이트만). DS는 "Story 8.4 polish forward"로 deferred 등록했으나 **CR Auditor가 Story 8.4 sprint plan에 jest/RNTL 인프라 scope 부재 검증** — 본 defer는 정식 home 없음. 백엔드 테스트가 핵심 로직(register_push_token user-swap atomic / validation / idempotent revoke) 커버 + mobile은 tsc + Zod 런타임 가드 보유로 부분 보완. **결정 필요**: (a) Story 8.4 ACs에 jest 인프라 도입 명시 추가 (b) 신규 dedicated story 생성 (c) 영구 dismiss + 백엔드 테스트 + tsc/Zod로 충분 명시. 현 시점 (a)/(b) 권장 — 포트폴리오 영업 시 mobile testing 신호 가치.
