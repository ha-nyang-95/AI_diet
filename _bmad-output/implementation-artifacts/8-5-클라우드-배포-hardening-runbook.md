...
| 2026-05-12 | **Story 추가 scope: Anthropic 제거** (사용자 요청) | `anthropic_adapter.py` + `test_anthropic_adapter.py` 삭제, `llm_router.py`를 OpenAI 단독 router로 단순화(cache + tenacity retry 보존). `config.py:anthropic_api_key`/`llm_fallback_model` 제거, `FeedbackOutput.used_llm` Literal에서 `"claude"`/`"claude-haiku-4-5"` 제거, `meal_analyses.used_llm` CHECK 제약 갱신 alembic 0023 추가(`'gpt-4o-mini','gpt-4o','stub'`로 swap + 기존 `'claude'` row를 `'gpt-4o-mini'`로 정리), `pyproject.toml`에서 `anthropic`/`langchain-anthropic` dep 제거, `daily-cost-alert.yml` Anthropic step 제거, `.env.example`/`.env.production.example` 정리, 3종 runbook 갱신(secret-rotation §2 deprecated, incident-response §1/§5 갱신). 1376 pytest passed(-3 = anthropic adapter test 삭제). ruff/mypy 0 errors. |
