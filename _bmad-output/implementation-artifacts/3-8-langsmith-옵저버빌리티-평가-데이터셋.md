...
2. **AC2 — NFR-S5 마스킹 키셋 SOT 단일화 (NFR-O2, NFR-S5 결합)**: `app/core/observability.py:_LANGSMITH_MASKED_KEYS: Final[frozenset[str]]` 정의. 키 set:
   ...
   - **신규 (LangGraph state 추가)**: `weight_kg`, `height_cm`, `allergies`, `parsed_items`, `food_items`, `clarification_options`(사용자가 선택한 음식명 → 식습관 추론 PII), `feedback_text`(전체 본문 — citation/macro 카드는 송신 OK이나 자연어 본문은 PII 가능). *Note*: `feedback`/`text`는 LangGraph state의 nested `feedback.text`라 *키 매칭만으로는 잡지 못함* — recursive 순회 + 부모 key path 무관(`text` 키 단독 매칭 시 false-positive 다수 — citation `text`도 매칭됨). **결정**: nested `feedback.text` 마스킹은 *상위 `feedback` dict 전체*를 `feedback_text` 키 별칭으로 추가 매칭하는 패턴 — 즉 dict access 시 `key in {"feedback"}`이면 redact dict 전체. 단순화 위해 `feedback` 키도 mask 대상에 추가(NFR-S5 conservative — coaching 자연어 본문도 PII 가능 잠재 보호).
   ...
   - 단위 테스트(`tests/core/test_observability.py`)에서 합집합 무결성 + Sentry 변경 시 LangSmith가 따라가는지 검증(import 후 set 비교).
...
- [ ] **Task 1 — `langsmith>=0.7.31` SDK bump (AC4)**
  - [ ] 1.1 `api/pyproject.toml` `dependencies`의 `"langsmith>=0.3"` → `"langsmith>=0.7.31"` 갱신.
  - [ ] 1.2 `cd api && uv sync --upgrade-package langsmith` 실행 → `uv.lock` 갱신.
  - [ ] 1.3 `tests/test_dependency_versions.py` 신규 — 2 케이스(langsmith ≥ 0.7.31 + tuple 비교 가드).
  - [ ] 1.4 `mypy app tests` 0 에러 — langsmith 0.7+ stub 변경 시 mypy override 추가 검토(`pyproject.toml [tool.mypy.overrides] module = ["langsmith.*"] ignore_missing_imports = true` 이미 있는지 확인 + 부재 시 추가).

- [ ] **Task 2 — `app/core/observability.py` 신규 + 마스킹 hook (AC1, AC2, AC3)**
  - [ ] 2.1 `api/app/core/observability.py` 신규.
  - [ ] 2.2 module-level imports — `from typing import Any, Final`, `import structlog`, `from langsmith import Client`, `from app.core.config import settings`, `from app.core.sentry import _MASKED_KEYS as _SENTRY_MASKED_KEYS`.
  - [ ] 2.3 `_LANGSMITH_EXTRA_KEYS: Final[frozenset[str]] = frozenset({"weight_kg", "height_cm", "allergies", "parsed_items", "food_items", "clarification_options", "feedback_text", "feedback"})` (LangGraph state 추가 PII 키).
  - [ ] 2.4 `_LANGSMITH_MASKED_KEYS: Final[frozenset[str]] = _SENTRY_MASKED_KEYS | _LANGSMITH_EXTRA_KEYS` (합집합 SOT).
  - [ ] 2.5 `_MASK_PLACEHOLDER: Final[str] = "***"` (sentry.py와 동일 placeholder).
  - [ ] 2.6 `_mask_recursive(value: Any) -> Any` private helper — dict는 키별 redact + 재귀, list는 element별 재귀, scalar는 통과. `sentry.py:_mask_dict` 패턴 확장(list of dict 처리 포함, sentry CR Gemini G3 정합).
  - [ ] 2.7 `def mask_run_inputs(inputs: dict[str, Any]) -> dict[str, Any]:` — `inputs`가 None이면 빈 dict 반환, dict이면 `_mask_recursive(inputs)` 결과 반환. 원본 미수정.
  - [ ] 2.8 `def mask_run_outputs(outputs: dict[str, Any]) -> dict[str, Any]:` — `mask_run_inputs`와 동일 구현(키셋 + 동작 동일). 별 시그니처는 LangSmith client `hide_inputs`/`hide_outputs` 콜백 시그니처 정합.
  - [ ] 2.9 module-level `_langsmith_client: Client | None = None` singleton 변수.
  - [ ] 2.10 `def init_langsmith() -> Client | None:` — `settings.langchain_tracing_v2`가 False면 log INFO `observability.langsmith.disabled` + `None` return + module 변수 None 유지. `True` + `langsmith_api_key == ""` → log WARNING `observability.langsmith.misconfigured` + None return. 정상 → `Client(api_key=settings.langsmith_api_key, hide_inputs=mask_run_inputs, hide_outputs=mask_run_outputs)` 생성 + module 변수 할당 + log INFO `observability.langsmith.initialized` (project=settings.langsmith_project).
  - [ ] 2.11 `def get_langsmith_client() -> Client | None:` — module 변수 반환.
  - [ ] 2.12 `def reset_langsmith_for_tests() -> None:` — module 변수 None 강제(테스트 monkeypatch fallback).

- [ ] **Task 3 — `main.py` lifespan에 init_langsmith() 호출 (AC1, AC3)**
  - [ ] 3.1 `api/app/main.py` import 추가 — `from app.core.observability import init_langsmith`.
  - [ ] 3.2 lifespan 함수 line 82(`init_sentry()`) *직후* 라인 — `init_langsmith()` 호출 추가. 1줄 추가 + try/except 불필요(`init_langsmith` 자체가 graceful — exception 발생 가능 분기는 langsmith Client init 자체 — settings 검증 후 호출이라 거의 발생 X, 만일 발생 시 lifespan 자체가 crash → main.py 87-145 패턴(`try/except + log.error + None`) 적용 검토 — *결정 D5*: `init_langsmith()` 자체가 None return으로 graceful이므로 wrapping try/except 추가 — 안전망(SDK 내부 unexpected ValueError 대비).
  - [ ] 3.3 *try/except 패턴*:
    ```python
    try:
        init_langsmith()
...
- [ ] **Task 4 — 단위 테스트 `tests/core/test_observability.py` (AC1, AC2, AC7)**
  - [ ] 4.1 `api/tests/core/test_observability.py` 신규.
  - [ ] 4.2 `tests/core/__init__.py` 빈 파일 신규(이미 있으면 skip — pytest 자동 디스커버리는 init 무관이지만 mypy 패키지 인식 정합).
  - [ ] 4.3 케이스 ≥ 12:
...
- [ ] **Task 5 — 통합 테스트 `tests/core/test_observability_tracing.py` (AC3)**
  - [ ] 5.1 `tests/core/test_observability_tracing.py` 신규.
  - [ ] 5.2 케이스 ≥ 3:
...
- [ ] **Task 6 — `scripts/upload_eval_dataset.py` 신규 (AC6)**
  - [ ] 6.1 `scripts/upload_eval_dataset.py` 신규(repo root 기준 `scripts/` — `seed_food_db.py` / `seed_guidelines.py` 패턴 정합).
  - [ ] 6.2 shebang `#!/usr/bin/env python3` + module docstring(스크립트 사용법 + AC6 명시).
  - [ ] 6.3 `__main__` block + argparse 4 옵션(`--dataset-name` / `--input-path` / `--description` / `--force` / `--dry-run`).
  - [ ] 6.4 `_load_dataset(path: Path) -> list[dict[str, str]]` — JSON 파일 로드 + 100건 + schema 검증(`input` / `expected_canonical` / `expected_path` / `category` 4 키 강제, 불일치 시 ValueError).
  - [ ] 6.5 `_build_examples(rows: list[dict[str, str]]) -> list[dict[str, Any]]` — LangSmith Example schema 변환(inputs/outputs/metadata 분리).
  - [ ] 6.6 `_upload(client, dataset_name, description, examples, force) -> int` — dataset 생성 또는 read fallback + examples 삽입. 기존 examples 존재 + force=False → skip(0 return). 정상 삽입 → len(examples) return.
  - [ ] 6.7 `--dry-run` → `client = None` + log INFO + payload structure print + sys.exit(0).
  - [ ] 6.8 `LANGSMITH_API_KEY` 미설정 → stderr `"LANGSMITH_API_KEY required — set in env before run"` + sys.exit(1).
  - [ ] 6.9 단위 테스트 `tests/scripts/test_upload_eval_dataset.py` 신규 — 5 케이스(AC6 명시):
...
- [ ] **Task 7 — `.env.example` 갱신 (AC5)**
  - [ ] 7.1 `.env.example` line 14-17 갱신:
    - `LANGSMITH_API_KEY=` (값 비움).
    - `LANGSMITH_PROJECT=balancenote-dev`.
    - `LANGCHAIN_TRACING_V2=false` (기존 `=true` → `=false`).
  - [ ] 7.2 주석 추가 — `# 로컬 개발은 false 디폴트. 스테이징/프로덕션은 Railway secrets에서 true 강제. 키 회전: docs/runbook/secret-rotation.md.`

- [ ] **Task 8 — 개인정보처리방침 갱신 (AC9)**
  - [ ] 8.1 `api/app/domain/legal_documents.py` `PRIVACY_POLICY_KO` 본문에 *제3자 제공·국외 이전* 단락 1줄 추가(AC9 본문 verbatim).
  - [ ] 8.2 `PRIVACY_POLICY_EN` 동일 단락 영문 추가(`docs/sop/03-disclaimer-ko-en.md` 패턴 정합).
  - [ ] 8.3 `PRIVACY_POLICY_VERSION` bump (예: `"1.0.0"` → `"1.1.0"`).
  - [ ] 8.4 `tests/domain/test_legal_documents.py` 갱신 — 2 케이스:
    - `PRIVACY_POLICY_KO` 본문에 `"LangSmith"` substring 포함.
    - `PRIVACY_POLICY_VERSION` ≥ 직전 baseline.
  - [ ] 8.5 *기존 사용자 재동의 강제는 OUT* — Story 1.4 ConsentVersionMismatchError 분기는 *major version bump*시만 — 본 스토리는 minor bump이라 강제 재동의 미발동(legal compliance 검토 — *제3자 제공* 단락은 정보주체 권리 영향 *낮음*: PII 송신 0이라 명시).

- [ ] **Task 9 — `docs/observability/langsmith-dashboard.md` + `docs/runbook/secret-rotation.md` 갱신 (AC10, AC11)**
  - [ ] 9.1 `docs/observability/` 디렉터리 신규 + `langsmith-dashboard.md` 신규(AC10 본문 구조 정합).
  - [ ] 9.2 `docs/runbook/secret-rotation.md`(이미 존재 — Story 1.1 architecture line 607)에 *LangSmith API key 회전 절차* 단락 추가(AC11 5단계).
  - [ ] 9.3 양 문서 cross-link — dashboard → secret-rotation, secret-rotation → dashboard.

- [ ] **Task 10 — `.github/workflows/ci.yml` `langsmith-eval-regression` job (AC8)**
  - [ ] 10.1 `.github/workflows/ci.yml`에 신규 job `langsmith-eval-regression` 추가:
    ```yaml
    ...
    ```
  - [ ] 10.2 GitHub repo *Settings → Secrets and variables → Actions*에 `LANGSMITH_API_KEY` 추가(운영자 SOP — 본 스토리는 *코드만 박고* secret은 운영자가 1회 설정).
  - [ ] 10.3 `langsmith-eval` 라벨 정의 — *수동 추가*. 본 스토리는 라벨 정의 자동화 OUT(GitHub UI에서 1회 생성 + 라벨 description *"Trigger LangSmith regression eval (T3 KPI 90%) — Free tier 5K trace/month 보호 라벨"*).

- [ ] **Task 11 — 회귀 가드 검증 + 운영 hardening (AC12)**
  - [ ] 11.1 `cd api && uv run pytest -q` 전체 통과 — Story 3.7 baseline *725 passed/9 skipped*에 *13~16 신규 테스트* 추가 후 *741~744 passed* 예상. coverage ≥ 70% 유지(`--cov-fail-under=70`).
  - [ ] 11.2 `cd api && uv run ruff check .` + `uv run ruff format --check .` + `uv run mypy app tests` 0 에러.
  - [ ] 11.3 `cd mobile && pnpm tsc --noEmit` 0 에러(코드 변경 0이라 자연 통과).
  - [ ] 11.4 `cd web && pnpm tsc --noEmit` 0 에러(legal page 추가 단락 변경 시 본 스토리는 백엔드 SOT 변경만 — privacy page는 백엔드 `/v1/legal/privacy` API 호출이 자동 갱신, web 정적 페이지 변경 0).
  - [ ] 11.5 `app/api/v1/analysis.py` / `app/services/analysis_service.py` / `app/graph/pipeline.py` / `app/graph/nodes/*.py` git diff 0 — 본 스토리는 라우터/서비스/노드 변경 X 강제(D1 정합).
  - [ ] 11.6 Story 3.3 / 3.4 / 3.5 / 3.6 / 3.7 모든 기존 테스트 회귀 0 — 특히 `test_pipeline.py` / `test_self_rag.py` / `test_evaluate_fit.py` / `test_generate_feedback.py` / `test_llm_router.py` / `test_analysis_stream.py` / `test_analysis_polling.py` / `test_analysis_clarify.py` 통과.
  - [ ] 11.7 `tests/core/test_sentry_mask.py`(Story 3.6) 회귀 0 — `_MASKED_KEYS` import는 본 스토리에서 read-only이라 회귀 위험 0. 단, `_LANGSMITH_MASKED_KEYS`가 합집합이라 *Sentry 키 제거 PR이 LangSmith에서도 자동 제거되는* 결합도 — `tests/core/test_observability.py:test_masked_keys_includes_sentry_keyset` 가드(Task 4.3).

- [ ] **Task 12 — 최종 verification + LangSmith 보드 placeholder 갱신 (수동, AC10)**
  - [ ] 12.1 운영자(hwan)가 LangSmith UI에 `balancenote-dev` / `balancenote-staging` / `balancenote-prod` 프로젝트 3개 생성. dev는 *Share → Public read-only* 토글(데모용). prod는 staging까지만 share — prod 보드는 외주 client별 옵션(architecture line 241 정합).
  - [ ] 12.2 `docs/observability/langsmith-dashboard.md` 보드 URL placeholder 채움(W3 종료 시점). PR 시점에는 TBD 1줄 OK — Story 8.6 영업 데모 페이지 작성 단계까지 완료 후 wire.
  - [ ] 12.3 1회 분석 실 호출 — `LANGCHAIN_TRACING_V2=true` 로컬 dev에서 `POST /v1/analysis/stream` 호출 → LangSmith 보드에 trace 도착 확인 + inputs/outputs에 `raw_text` / `weight_kg` 등 PII가 `"***"`로 redact 됐음 *시각 확인*. 결과 스크린샷 → `docs/observability/langsmith-dashboard.md`에 첨부(Story 8.6 영업 자료 forward-compat).
...
claude-opus-4-7[1m] (Story Context Engine — bmad-create-story v6.5.0)
...
- 본 스토리는 *cross-cutting infrastructure*(observability + script + 문서 + CI)만 추가. 라우터/서비스/노드 변경 0(D4 정합).
- NFR-S5 마스킹 SOT는 `app/core/sentry.py:_MASKED_KEYS` import 합집합 — 1지점 변경이 양쪽 hook 자동 반영(D1 정합).
- LangGraph native tracing은 `LANGCHAIN_TRACING_V2=true` env 1개로 활성 — 코드 wiring 0.
- `langsmith>=0.7.31` 보안 패치 bump 강제(AC4) — GHSA-rr7j-v2q5-chgv `events` 배열 leak 차단.
