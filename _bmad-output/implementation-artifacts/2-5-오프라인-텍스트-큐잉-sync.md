...
3. **AC3 — `mobile/lib/offline-queue.ts` 신규 — AsyncStorage FIFO 큐 + 지수 backoff + mutex**
   **Given** 모바일 텍스트 식단 큐잉 요구, **When** `enqueueMealCreate(item)` / `flushQueue()` / `getQueueSnapshot()` / `removeQueueItem(client_id)` 호출, **Then**:
     ...
     - **지수 backoff scheduling (CR P8 정정 baseline)**: `flushQueue`는 *호출자 책임*이지만 *항목 단위 backoff*은 큐 자체 — *총 3회 송신* (`BACKOFF_BASE_MS = [0, 1000, 2000]`): `attempts=0` 즉시 시도, `attempts=1` 1초 + jitter(±20%), `attempts=2` 2초 + jitter. `attempts >= 3`은 retry 미수행(*"3회 재시도 실패 — 큐 잔존"*) — AC11 smoke #4 *"0→1→2→3"* 정합. ~~`attempts=3` 4초~~는 원안 자기모순(retry 미수행 + 4초 wait 동시 명시) 흡수 — 본 baseline은 3회 send cap.
     - **mutex (per-userId single-flight, CR P1 정정 baseline)**: 동시 `flushQueue` 호출 race 차단 — module-level `flushingByUser: Map<string, Promise<FlushResult>>` (cross-user lock leak 차단). 진행 중 호출은 같은 promise 반환. R-M-W 직렬화는 별도 `_userLocks: Map<string, Promise<unknown>>` chain(CR P2).
