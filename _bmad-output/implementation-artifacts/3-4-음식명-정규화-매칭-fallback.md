...
- [x] **Task 1 — `app/graph/state.py` 확장 (AC: #1)**
  - [x] 1.1 `ClarificationOption` Pydantic BaseModel 정의 (`label`/`value`).
  - [x] 1.2 `MealAnalysisState`에 `clarification_options: list[ClarificationOption]` 필드 추가.
  - [x] 1.3 `tests/graph/test_state.py` 갱신(label/value 검증 + extra forbid + max_length).

- [x] **Task 2 — `app/adapters/openai_adapter.py` 확장 (AC: #2)**
  - [x] 2.1 `RewriteVariants` + `ClarificationVariants` Pydantic 모델 정의.
  - [x] 2.2 `parse_meal_text(text)` 함수 + system 프롬프트 + structured output.
  - [x] 2.3 `rewrite_food_query(query)` 함수.
  - [x] 2.4 `generate_clarification_options(query)` 함수.
  - [x] 2.5 `tests/test_openai_adapter.py` 갱신/신규(mock 5 케이스 — happy/transient/permanent/empty/max_options).

- [x] **Task 3 — `app/rag/food/normalize.py` 신규 (AC: #3)**
  - [x] 3.1 `lookup_canonical(session, alias)` async 함수 + 빈 입력 가드.
  - [x] 3.2 `tests/rag/test_food_normalize.py` 신규(hit/miss/empty/exact-match-only).

- [x] **Task 4 — `app/rag/food/search.py` 신규 (AC: #4)**
  - [x] 4.1 `search_by_name(session, name)` exact + score 1.0.
  - [x] 4.2 `search_by_embedding(session, query, top_k)` HNSW + score 정규화.
  - [x] 4.3 `compute_retrieval_confidence(matches, single_item)` aggregator.
  - [x] 4.4 `tests/rag/test_food_search.py` 신규(exact + HNSW + min/single 분기 + empty index graceful).

- [x] **Task 5 — `app/graph/nodes/parse_meal.py` 갱신 (AC: #5)**
  - [x] 5.1 `meals.parsed_items` 우선 읽기 분기.
  - [x] 5.2 `parse_meal_text` LLM fallback.
  - [x] 5.3 빈 입력 graceful.
  - [x] 5.4 `tests/graph/nodes/test_parse_meal.py` 갱신(DB 우선 + LLM fallback + 빈 입력 + transient retry + ValidationError).

- [x] **Task 6 — `app/graph/nodes/retrieve_nutrition.py` 갱신 (AC: #6)**
  - [x] 6.1 빈 `parsed_items` 분기.
  - [x] 6.2 alias → exact → HNSW 3단 검색.
  - [x] 6.3 `rewrite_attempts > 0` 분기(rewritten_query 사용).
  - [x] 6.4 sentinel 분기 + boost(Story 3.3 회귀).
  - [x] 6.5 multi-item compute_retrieval_confidence(min).
  - [x] 6.6 `tests/graph/nodes/test_retrieve_nutrition.py` 갱신(8 케이스).

- [x] **Task 7 — `app/graph/nodes/rewrite_query.py` 갱신 (AC: #7)**
  - [x] 7.1 `rewrite_food_query` 호출 + 변형 join.
  - [x] 7.2 빈 입력 graceful.
  - [x] 7.3 `rewrite_attempts` increment.
  - [x] 7.4 `tests/graph/nodes/test_rewrite_query.py` 갱신(LLM mock + transient + 무한 루프 회귀).

- [x] **Task 8 — `app/graph/nodes/evaluate_retrieval_quality.py` + `edges.py` 갱신 (AC: #8)**
  - [x] 8.1 `EvaluationDecision.route` Literal 3-way 확장 (`"rewrite"`/`"continue"`/`"clarify"`).
  - [x] 8.2 `evaluate_retrieval_quality`에 `attempts >= 1 + low_confidence` → `"clarify"` 분기.
  - [x] 8.3 `route_after_evaluate` 라우터 3-way 분기 + `"request_clarification"` 매핑.
  - [x] 8.4 `tests/graph/nodes/test_evaluate_retrieval_quality.py` 갱신(3-way 분기 + Story 3.3 회귀).
  - [x] 8.5 `tests/graph/test_edges.py` 갱신(clarify 분기 + fail-soft).

- [x] **Task 9 — `app/graph/nodes/request_clarification.py` 신규 + `pipeline.py` 갱신 (AC: #9)**
  - [x] 9.1 `request_clarification` 노드 함수 + graceful 빈 옵션.
  - [x] 9.2 `nodes/__init__.py` export 갱신.
  - [x] 9.3 `pipeline.py`에 `request_clarification` 노드 등록 + END edge.
  - [x] 9.4 `add_conditional_edges` 매핑 3-way 갱신.
  - [x] 9.5 `tests/graph/nodes/test_request_clarification.py` 신규(4 케이스).

- [x] **Task 10 — `app/services/analysis_service.py` `aresume` 강화 (AC: #10)**
  - [x] 10.1 `langgraph.types.Command` import + `update`/`goto` 패턴 적용.
  - [x] 10.2 `user_input` 검증(`raw_text_clarified` 또는 `selected_value` 필수).
  - [x] 10.3 Sentry transaction `meal_analysis_resume`.
  - [x] 10.4 `tests/services/test_analysis_service.py` 갱신(clarification → aresume → 정상 종단 + 빈 input ValueError).

- [x] **Task 11 — config + 데이터셋 (AC: #11)**
  - [x] 11.1 `app/core/config.py`에 `food_retrieval_top_k` + `clarification_max_options` 추가.
  - [x] 11.2 `api/tests/data/korean_foods_100.json` 신규 (100 행 + 분포 명시).
  - [x] 11.3 `tests/test_config.py` 갱신(env override).
  - [x] 11.4 `tests/test_korean_foods_dataset.py` 신규(schema + 분포 검증).
  - [x] 11.5 `tests/test_korean_foods_dataset.py`에 *시드된 `food_nutrition.name`에 `expected_canonical` 모두 존재* 검증 추가(orphan 차단 — `RUN_DATASET_SEED_CHECK=1` 게이트).

- [x] **Task 12 — T3 KPI eval 테스트 (AC: #12)**
  - [x] 12.1 `tests/test_korean_foods_eval.py` 신규 (`@pytest.mark.eval`).
  - [x] 12.2 `api/pyproject.toml [tool.pytest.ini_options].markers`에 `eval` 추가(line 95-97 `perf` 패턴 정합).
  - [x] 12.3 `RUN_EVAL_TESTS=1` 게이트 환경 변수 분기.
  - [x] 12.4 dev mock LLM / staging 실 LLM 분기 문서화(테스트 docstring).

- [x] **Task 13 — perf + 게이트 + commit/push (AC: #13)**
  - [x] 13.1 `tests/rag/test_food_search_perf.py` 신규(`@pytest.mark.perf`).
  - [x] 13.2 `tests/graph/test_pipeline_perf.py` budget 갱신(이미 NFR_P4_BUDGET_SECONDS=3.0/NFR_P5_BUDGET_DELTA_SECONDS=2.0 — Story 3.4 baseline 정합 확인).
  - [x] 13.3 `cd api && uv run ruff check . && uv run ruff format --check .` 통과.
  - [x] 13.4 `cd api && uv run mypy app` 0 에러 회귀.
  - [x] 13.5 `cd api && uv run pytest` 461 passed/8 skipped/0 failed + coverage 86.86%(>= 70 게이트 통과).
  - [x] 13.6 sprint-status `3-4-* → in-progress`(DS 시작 시).
  - [x] 13.7 sprint-status `3-4-* → review`(DS 종료 시 + last_updated 갱신).
  - [x] 13.8 단일 commit + push (`feat(story-3.4): ...`).
...
- LLM 어댑터 monkeypatch 시 `app.graph.nodes.<name>` 모듈 attribute access가 `__init__.py`의 함수 export로 가려짐 → 테스트에서 `sys.modules["app.graph.nodes.<name>"]` 직접 lookup 또는 string 경로 monkeypatch로 우회.
- Story 3.3 baseline 회귀: `parse_meal`/`retrieve_nutrition`/`rewrite_query`가 실 LLM 호출로 전환되어 stub 가정 테스트(`test_pipeline.py` / `test_self_rag.py` / `test_analysis_service.py` / `test_main_lifespan.py`) 모두 deterministic mock fixture(`_mock_llm_adapters`) 추가 + sentinel 입력으로 갱신.
- `Command(update=..., goto=...)` Pydantic v2 generic — mypy strict 통과 위해 `Command[Any]` 명시.

### Completion Notes List

- Story 3.4 13개 AC + 13개 Task 모두 완료. `food_aliases` 1차 lookup → `food_nutrition` exact 매칭 → HNSW top-3 임베딩 fallback 3단 검색 + Self-RAG 1회 한도 후 `clarify` 분기 + `analysis_service.aresume` Command 패턴 + 한식 100건 ground-truth 데이터셋 + T3 KPI eval marker 박힘.
- 게이트 통과: ruff/format/mypy strict 0 에러 + pytest **461 passed / 8 skipped(perf+eval+seed) / 0 failed** + coverage **86.86%**(>= 70 게이트 통과). Story 3.3 baseline 404 + 30 = 434+ 목표 초과 461.
- 신규 모듈 coverage: `normalize.py` 100%, `search.py` 97%, `request_clarification` 88%+. spec 임계값(≥ 80%) 충족.
- Story 3.5/3.6/3.7/3.8 OUT 정합 — `evaluate_fit`은 stub(`fit_score=50`) 보존, `generate_feedback`은 stub(`text="(분석 준비 중)"`) 보존, SSE 라우터/PIPA 게이트/LangSmith mask_run/Redis cache/Anthropic dual-LLM 미진행.
- T3 KPI 측정 인프라(`@pytest.mark.eval` skip default + `RUN_EVAL_TESTS=1` 게이트) 박힘. 실 90% 측정은 Story 3.8 W3 슬롯에서 LangSmith eval로 검증 — 본 스토리는 *데이터 SOT + 측정 인프라*가 통과 기준.

### File List

신규:
- `api/app/rag/food/normalize.py`
- `api/app/rag/food/search.py`
- `api/app/graph/nodes/request_clarification.py`
- `api/tests/data/korean_foods_100.json`
- `api/tests/rag/test_food_normalize.py`
- `api/tests/rag/test_food_search.py`
- `api/tests/rag/test_food_search_perf.py`
- `api/tests/graph/nodes/test_request_clarification.py`
- `api/tests/test_config.py`
- `api/tests/test_korean_foods_dataset.py`
- `api/tests/test_korean_foods_eval.py`

갱신:
- `api/app/adapters/openai_adapter.py` — `parse_meal_text` + `rewrite_food_query` + `generate_clarification_options` + `RewriteVariants`/`ClarificationVariants` Pydantic.
- `api/app/graph/state.py` — `ClarificationOption` Pydantic + `MealAnalysisState.clarification_options` + `EvaluationDecision.route` 3-way Literal.
- `api/app/graph/nodes/parse_meal.py` — DB 우선 + LLM fallback 실 비즈니스 로직.
- `api/app/graph/nodes/retrieve_nutrition.py` — alias → exact → HNSW 3단 + sentinel 회귀 보존 + multi-item min.
- `api/app/graph/nodes/rewrite_query.py` — 실 LLM rewrite + attempts increment.
- `api/app/graph/nodes/evaluate_retrieval_quality.py` — `clarify` 분기 추가 (3-way).
- `api/app/graph/edges.py` — `route_after_evaluate` 3-way 라우터.
- `api/app/graph/pipeline.py` — `request_clarification` 노드 등록 + END edge + 3-way conditional edges.
- `api/app/graph/nodes/__init__.py` — `request_clarification` export.
- `api/app/services/analysis_service.py` — `aresume` Command(update, goto="parse_meal") 패턴.
- `api/app/core/config.py` — `food_retrieval_top_k` + `clarification_max_options`.
- `api/pyproject.toml` — `eval` marker 등재.
- `api/tests/graph/test_state.py` — ClarificationOption + 3-way Literal 테스트.
- `api/tests/test_openai_adapter.py` — Story 3.4 LLM 함수 3종 mock 케이스.
- `api/tests/graph/nodes/test_parse_meal.py` — 5 케이스 갱신.
- `api/tests/graph/nodes/test_retrieve_nutrition.py` — 8 케이스 갱신.
- `api/tests/graph/nodes/test_rewrite_query.py` — LLM mock 케이스 갱신.
- `api/tests/graph/nodes/test_evaluate_retrieval_quality.py` — 3-way 분기 회귀.
- `api/tests/graph/test_edges.py` — `clarify` 분기 추가.
- `api/tests/graph/test_pipeline.py` — LLM mock fixture 추가 + sentinel 입력 갱신.
- `api/tests/graph/test_self_rag.py` — clarify 분기 추가 + LLM mock fixture.
- `api/tests/services/test_analysis_service.py` — aresume Command 검증 + LLM mock fixture.
- `api/tests/test_main_lifespan.py` — LLM mock fixture + sentinel 입력 갱신.
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — `3-4-* → review` + last_updated 갱신.
