...
> 2026-05-03 CR (Amelia) — Blind Hunter / Edge Case Hunter / Acceptance Auditor 3-layer 적대적 리뷰. Triage: 2 decision-needed / 17 patch / 6 defer / 11 dismissed. **모든 19 패치 적용 완료** + ruff/format/mypy 0 에러 + 608 passed / 8 skipped / 0 failed + coverage 86.69%.

**Decision-needed → 패치 적용 (D-1 (a), D-2 (b)):**

- [x] [Review][Decision][Patch] **D-1 단위 의미 모순 → (a) per-100g 통일 적용** — `_quantity_multiplier`가 모든 단위를 *grams / 100*으로 환산: `"1kg"` → 10, `"200g"` → 2.0, `"1인분"` → 2.5(`_KOREAN_SERVING_GRAMS_ESTIMATE = 250.0` 추정). `RetrievedFood.nutrition` 100g 베이시스 정합. 테스트 데이터 per-100g 값으로 갱신.
- [x] [Review][Decision][Patch] **D-2 fit_label band taxonomy 충돌 → (b) 어댑터 helper 추가 적용** — `app/domain/fit_score.py:to_summary_label(fit_label, fit_score) -> SummaryFitLabel` 신규. domain 4-band(`good/caution/needs_adjust/allergen_violation`) ↔ presentation 5-band(`allergen_violation/low/moderate/good/excellent`) 매핑. `app/graph/state.py`에서 `as` 구문으로 explicit re-export. 두 SOT 의식적 분리 — Story 3.6/4.x wiring 시 helper 경유.

**Patch — 모두 적용 (BLOCKER 3 + MAJOR 5 + MINOR 9):**

- [x] [Review][Patch] **B-1 메밀(buckwheat) substring → 밀(wheat) cross-allergen false-positive** — `_LABEL_SUBSTRING_EXCLUSIONS = {"밀": ("메밀",)}` 도입. 메밀 substring을 텍스트에서 먼저 제거 후 밀 매칭. 회귀 가드 `test_buckwheat_does_not_trigger_wheat_allergy` + `test_buckwheat_food_with_buckwheat_allergy_still_triggers` 추가.
- [x] [Review][Patch] **B-2 `넛` alias가 도넛/코코넛에 fire → 호두 false-positive** — `_ALIAS_TEXT_EXCLUSIONS = {"넛": ("도넛", "코코넛")}` 도입. 회귀 가드 `test_donut_does_not_trigger_walnut_allergy` + `test_coconut_does_not_trigger_walnut_allergy` + `test_walnut_food_with_walnut_allergy_still_triggers` 추가.
- [x] [Review][Patch] **B-3 `기타` 22-라벨 substring이 식약처 카테고리 `기타가공식품`에 fire** — `_SKIP_SUBSTRING_LABELS = frozenset({"기타"})` 도입(22-라벨 substring 루프에서 스킵). `ALLERGEN_ALIAS_MAP`에 `"기타알레르기성분" → "기타"` explicit alias 추가(AC5 22-case 보존). 회귀 가드 3건 추가.
- [x] [Review][Patch] **M-1 `caplog`로 structlog 미캡처 → masking 테스트 vacuous** — `structlog.testing.capture_logs()`로 교체. allergen short-circuit 경로 + happy-path BMR 계산 경로 2 테스트 + `_FORBIDDEN_LOG_KEYS` key-level 검증 + value substring fail-safe.
- [x] [Review][Patch] **M-2 빈 `parsed_items[].name`이 모든 retrieved_food와 substring 매칭** — `aggregate_meal_macros`에서 `if not item_name: continue` early guard + `state.py:FoodItem.name = Field(min_length=1)` Pydantic invariant.
- [x] [Review][Patch] **M-3 `_quantity_multiplier`에서 NaN/Inf 전파** — `math.isnan`/`isinf` 가드(g/kg/인분 3 분기 모두). 회귀 가드 `test_aggregate_meal_macros_quantity_nan_g_falls_back_to_1` 추가.
- [x] [Review][Patch] **M-4 `1kg` 수량이 1.0으로 silent 처리** — kg 분기 추가(`g` 분기보다 먼저 체크). 회귀 가드 `test_aggregate_meal_macros_quantity_kg_scales_x10` 추가.
- [x] [Review][Patch] **M-5 vacuous test `test_compute_fit_score_uses_macro_targets_lookup`** — `r1.components.macro != r2.components.macro or r1.components.calorie != r2.components.calorie` 단언으로 lookup 분기 입증.
- [x] [Review][Patch] **m-1 alias map 크기 가드** — `<= 14` → `<= 13`(B-3 fix로 `기타알레르기성분` 추가됨, spec 8-12 + 1).
- [x] [Review][Patch] **m-2 `_coerce_*` helper silent swallow** — 3 helper(`_coerce_profile`/`_coerce_food_item`/`_coerce_retrieved_food`) except에서 `log.warning("evaluate_fit.coerce_failed", kind=..., error=type(e).__name__)` 추가(masked).
- [x] [Review][Patch] **m-3 `violated_count` self-emitted 문자열 파싱** — `evaluate_fit.py`가 `len(fit_evaluation.reasons)`를 직접 사용(`compute_fit_score`가 violation 케이스에서 reasons를 violation 1건당 1줄로 채우므로 정합).
- [x] [Review][Patch] **m-4 `compute_fit_score` ValueError catch 부재** — BMR/TDEE 호출 try/except → `FitEvaluation(fit_reason="incomplete_data", reasons=["프로필 입력값 비정상 — BMR 산출 불가"])` graceful fallback.
- [x] [Review][Patch] **m-5 `reasons`가 mid-range 점수에 misleadingly positive** — `calorie_score < 25` (0~24) 구간에 `f"칼로리 권장 범위 ±{round(deviation_pct)}% 이탈"` reason append. neutral fallback("전반적으로 권장 범위에 부합")은 `total >= 80`일 때만, 아니면 "권장 범위에 부분 부합".
- [x] [Review][Patch] **m-6 BMR 음수 출력 silent** — `compute_bmr_mifflin`에 `if bmr <= 0: raise ValueError("invalid bmr inputs")` 가드. 회귀 가드 `test_bmr_extreme_inputs_yielding_negative_raises_value_error` 추가.
- [x] [Review][Patch] **m-7 NFC 정규화 부재 (KOREAN_22_ALLERGENS module init)** — `_KOREAN_22_ALLERGENS_NFC: tuple[str, ...]` module 로드 시 미리 생성, `detect_allergen_violations`가 NFC SOT 사용.
- [x] [Review][Patch] **m-8 legacy compat test 모순 상태 enshrine** — docstring 명시 갱신: `fit_label="needs_adjust"`는 *Pydantic default* 동작 검증만, fit_score(85)와 band-일관성을 시사하지 않음. 추후 model_validator 도입 시 의도적 깨짐 + legacy stub callsite 함께 정리 명시.
- [x] [Review][Patch] **m-9 clarify→aresume happy path `fit_score > 0` 단언 추가 (AC10)** — `test_aresume_resumes_from_parse_meal_with_clarified_text`에 `fe["fit_reason"] == "ok"` + `fe["fit_label"] != "allergen_violation"` + `0 <= fe["fit_score"] <= 100` 단언 추가.

**Defer (스토리 범위 외 / 사전 존재 / 명시 OUT):**

- [x] [Review][Defer] **df-1 hardcoded `sex="female"` for all users** [api/app/domain/fit_score.py:compute_fit_score] — 스펙이 Story 8.4 polish로 명시 deferred
- [x] [Review][Defer] **df-2 negation phrasing(`땅콩없음`/`우유 미함유`) substring false-positive** [api/app/domain/fit_score.py:detect_allergen_violations] — 스펙 posture "보수적 false-positive 우선 — 안전 측면", ingredient list 처리는 Story 4.x deferred
- [x] [Review][Defer] **df-3 `포크` alias가 비식품 단어(포크송/포크댄스/포크리프트)에 fire 가능** [api/app/domain/fit_score.py:ALLERGEN_ALIAS_MAP] — LLM/OCR 파이프라인이 비식품 음식명 emit 가능성 낮음, alias 보강은 Story 8.4 polish deferred
- [x] [Review][Defer] **df-4 tautological 22-allergen 테스트 (`아황산류`/`잔류 우유 단백`)** [api/tests/domain/test_fit_score_allergen_22.py] — 와인/카제인 alias 매핑 부재 — Story 8.4 alias map 50+ 확장 deferred
- [x] [Review][Defer] **df-5 detect_allergen_violations가 quantity 무시 (trace amount == primary ingredient)** [api/app/domain/fit_score.py:detect_allergen_violations] — 안전 baseline 의식적 선택, dose threshold는 Story 8.4 polish deferred
- [x] [Review][Defer] **df-6 BMR 검증이 ratio sanity 미체크 (age=150 + weight=1 → 음수 BMR)** [api/app/domain/bmr.py:compute_bmr_mifflin] — DB CHECK가 실제 SOT, 방어 layer는 m-6 패치로 음수 출력 가드 추가하면 충분 (m-6과 별개로 ratio 검증은 Story 8.4 polish deferred)

**Dismissed (false positive / 노이즈):**

`FitScoreComponents.allergen_violation()` vs `.zero()` byte-identical (의도 분리 OK), 알레르기 detection O(N×22) micro-perf, `nutrition` dict[str, str] 타입 widening (이미 채택), `band_for_score` 100 clamp 미존재 (오버슛 불가능), test_pipeline.py sentinel `fit_reason=="ok"` (테스트 통과), test_main_lifespan.py 미갱신 (스펙 조건문 vacuous compliance), data/README.md 경로 (Story 3.1/3.2/3.4 정합), compounded round() across 4 components (deterministic), KDRIS validator `>= 0.001` vs `> 0.001` (baseline 영향 X), `잔류 우유 단백` substring overlap with `우유` (스펙 안전 posture 정합), evaluate_fit + compute_fit_score 중복 incomplete_data 검사 (방어 OK).
...
| 2026-05-03 | Amelia (CR) | 3-layer 적대적 리뷰(Blind / Edge Case / Acceptance Auditor) 후 19 패치 일괄 적용 — D-1 quantity multiplier per-100g 통일(`1인분` → 2.5 추정) + D-2 `to_summary_label` 어댑터 helper(domain 4-band ↔ presentation 5-band) + B-1/B-2/B-3 substring false-positive 가드 3종(`_LABEL_SUBSTRING_EXCLUSIONS`/`_ALIAS_TEXT_EXCLUSIONS`/`_SKIP_SUBSTRING_LABELS` + `기타알레르기성분` alias) + M-1 `structlog.testing.capture_logs` 마스킹 테스트 + happy-path 마스킹 + M-2 빈 name 가드(`Field(min_length=1)`) + M-3/M-4 NaN/kg quantity 가드 + M-5 vacuous test 단언 보강 + m-1~m-9 minor 9건. 608 passed/8 skipped/0 failed + coverage 86.69% + ruff/format/mypy 0 에러. Story 3.3/3.4 회귀 0건. Status: review → done. |
