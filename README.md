...
Story 3.2 추가 — 한국 1차 출처 가이드라인 RAG 시드(KDRIs/KSSO/KDA/MFDS/KDCA 50+
chunks, `knowledge_chunks` + HNSW + GIN `applicable_health_goals`)도 `docker compose up`
1-cmd로 함께 자동 시드됩니다. `OPENAI_API_KEY` 미설정 시 dev 환경은 embedding NULL
graceful로 chunks 시드 진행, prod/staging 또는 `RUN_GUIDELINE_SEED_STRICT=1`은
fail-fast. 분기 1회 자료 갱신 + 식약처 22종 알레르기 검증 SOP는
`api/data/guidelines/README.md` 참조.
...
- `mobile/lib/offline-queue.ts` AsyncStorage FIFO 큐(`bn:offline-queue:meal:<user_id>` per-user
  namespace) + 100건 hard cap + per-userId 직렬화 mutex(R-M-W race 차단) + per-userId flush
  single-flight + `useOfflineQueue` hook + manual retry `resetAttempts(userId, clientId?)` 헬퍼.
- 백엔드 `POST /v1/meals` `Idempotency-Key` 헤더 처리 — `meals.idempotency_key` TEXT NULL 컬럼 +
  ...
  SELECT-INSERT-CATCH-SELECT(인덱스명 한정 catch) + 200 idempotent replay (RFC 9110, W46 흡수) +
  trim/empty 정규화.
- 자동 flush — `useOnlineStatus` offline → online 전이 시 `flushQueue` 자동 호출 + 지수 backoff
  *총 3회 송신*(`attempts=0` 즉시, `attempts=1` 1s, `attempts=2` 2s + jitter ±20%) → `attempts >= 3`
  stuck(큐 잔존 + manual retry는 attempts reset 후 재시도).
- 사진 입력 + PATCH/DELETE 오프라인 차단 (epic AC) — 사진은 R2/Vision 모두 *온라인 필수*, PATCH는
  *진입 차단*만(진입당 1회 Alert + back, Story 8 polish 시점에 PATCH 큐잉 검토 — DF43).
- 큐 size 100 hard cap — 초과 시 Alert *"오프라인 대기 항목이 너무 많아요(100+)"*. 4xx 영구 실패는
  400/422만(즉시 제거 + raw_text 발췌 Alert), 401은 flush abort, 429/5xx/네트워크는 transient(잔존 + backoff).
...
#### 5.4.4 해지 / 결제 이력 흐름 (Story 6.2)

**해지 흐름**

- ``POST /v1/payments/cancel``은 *Toss API 호출 X* — DB-only ``subscriptions.status='cancelled'``
  + ``cancelled_at`` set + cancel audit ``payment_logs`` row 1건 INSERT(``event_type='cancel'``,
  ``amount_krw=0``). epics.md:852 invariant — *"즉시 차단 X — 다음 결제일까지 활성"*.
- 사용자는 ``expires_at``까지 모든 기능 정상 이용. 다음 결제 주기 도래 시 Story 6.3 webhook이
  ``status='expired'`` 전이 + 자동 갱신 차단.
- ``GET /v1/payments/subscription``은 cancelled-but-not-expired 상태도 200 반환(Story 6.1
  deferred UX gap 흡수). 클라이언트는 ``status === "cancelled"`` 분기로 *"해지됨 — {expires_at}까지
  이용 가능"* 표시.
- **환불 자동화는 OUT(MVP)** — 외주 인수 후 클라이언트가 자체 환불 정책 + Toss
  ``POST /v1/payments/{paymentKey}/cancel`` API 통합(본 스토리 baseline은 1회 결제 + billing key
  미발급으로 자동 갱신 자체가 없으므로 즉시 환불은 사용자 권리 박탈 risk).

**결제 이력 흐름**

- ``GET /v1/payments/history?limit=20&cursor=...``는 ``payment_logs`` 테이블의 사용자 결제
  이력(시점·금액·상태·영수증 URL)을 *시계열 page*로 반환. 인덱스
  ``idx_payment_logs_user_id_occurred_at = (user_id, occurred_at DESC)`` hit.
- 영수증 URL은 Toss 응답 ``receipt.url`` 필드 보존 — ``payment_logs.raw_payload``에 마스킹 후
  영속화 + ``_extract_receipt_url`` 헬퍼가 ``https://``/``http://`` prefix 검증 후 노출(XSS hardening).
- page cursor는 ``(occurred_at, id)`` 복합 base64 인코딩 — 같은 ``occurred_at`` row가 다수일
  때도 ``id`` tie-breaker로 안정 정렬(Postgres native row-value compare).

**모바일 / Web 흐름**

- 모바일: ``(tabs)/settings/subscription.tsx`` 활성 카드에 *"구독 해지"* 버튼(Alert 확인) +
  ``(tabs)/settings/payment-history.tsx`` 신규 화면(``<FlatList>`` 1페이지).
- Web: ``/account/subscribe`` active 분기에 *"구독 해지"* 버튼(``window.confirm``) +
  ``/account/subscribe/history`` 신규 페이지(``<table>`` + *"더 보기"* cursor pagination).
- 영수증 외부 링크는 새 탭(``target="_blank"`` + ``rel="noopener noreferrer"``).

#### 5.4.5 webhook 수신 흐름 (Story 6.3)

Story 6.3에서 Toss webhook receipt + HMAC-SHA256 시그니처 검증 + Idempotency-Key 다층
멱등 + ``renew``/``failed``/``refund`` event 분기를 추가했다. 자동 갱신·실패·환불 이벤트가
*비동기*로 도착하므로 webhook이 결제 상태의 *후속 갱신* SOT(첫 결제는 Story 6.1 widget
success callback이 SOT).

**셋업 5단계**

1. Toss 콘솔 *"개발자 도구 > Webhook"*에서 신규 endpoint 등록 — URL
   ``https://<api-host>/v1/payments/webhook`` + 이벤트 4종 구독
   (``PAYMENT.STATUS_CHANGED`` / ``PAYMENT_FAILED`` / ``REFUND_COMPLETED`` / ``REFUND_PARTIAL``).
2. Toss 콘솔에서 발급되는 *웹훅 secret*(``whsec_*`` 또는 ``wsk_*`` prefix) 복사. **secret_key와
   별 키** — 혼동 회피.
3. ``.env``에 ``TOSS_WEBHOOK_SECRET_KEY=<whsec_...>`` 설정.
4. 컨테이너 재배포 — ``docker-compose up -d`` 또는 Railway redeploy.
5. Toss 콘솔에서 *"테스트 webhook 발송"* — 시그니처 검증 + DB 적용 정합 확인.

**시그니처 검증**

- ``TossPayments-Webhook-Signature: t=<unix>,v1=<base64_hmac_sha256>`` 헤더로 webhook 요청
  검증. HMAC-SHA256(secret, ``<unix>.<raw_body>``) + ``hmac.compare_digest``(timing-safe).
- timestamp drift 5분 초과 → 401 reject(replay attack 차단). Stripe 표준 정합.
- 미통과 시 sentry warning + 401 응답 + Toss 재시도 진입(Toss 표준 1분 backoff).

**이벤트 분기**

- ``PAYMENT.STATUS_CHANGED`` + ``data.status='DONE'``(renew 성공) → ``subscriptions.expires_at +=30일``
  + ``payment_logs`` ``event_type='renew'`` row INSERT.
- ``PAYMENT_FAILED``(renew 실패) → ``subscriptions`` 변경 X + ``event_type='failed'`` audit row +
  sentry warning(*dunning 정책 검토 신호*).
- ``REFUND_COMPLETED`` / ``REFUND_PARTIAL`` → ``subscriptions`` 변경 X + ``event_type='refund'``
  audit row(*즉시 환불은 OUT — Story 6.2 정합*).

**expires_at 만료 처리**

- webhook 도래 전(*Toss 미통보 케이스 안전판*) — ``app/workers/subscription_expire.py``
  APScheduler 매시 정각 KST sweep으로 ``expires_at < now() AND status='active'`` row를
  ``expired`` 전이(redis lock 30분 TTL + 100건 cap).
- ``GET /v1/payments/subscription`` lazy gate — 사용자 polling이 sweep 도달 전이라도
  ``(expires_at IS NULL OR expires_at > now())`` 게이트로 active-but-expired race window
  차단(DF135 흡수).

**멱등 처리**

- Idempotency 다층 게이트 — Header ``Idempotency-Key``(send 시) > body ``eventId``
  (``toss:event:<event_id>`` namespace) > ``payment_logs.idempotency_key`` partial UNIQUE
  (SELECT-INSERT-CATCH-SELECT race-free 패턴, Story 2.5/6.1 SOT 정합).
- 같은 ``event_id`` 재전송 → 200 OK ack + 추가 row INSERT X(*Toss retry 정합*).
